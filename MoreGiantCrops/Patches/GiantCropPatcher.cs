        ...
        *********/
        ...
        private static bool Before_Draw(GiantCrop __instance, SpriteBatch spriteBatch, Vector2 tileLocation, double ___shakeTimer)
        {
            try
            {
                ...
                {
                    spriteBatch.Draw(tex.Value, Game1.GlobalToLocal(Game1.viewport, tileLocation * 64f - new Vector2(___shakeTimer > 0.0 ? (float)(Math.Sin(2.0 * Math.PI / ___shakeTimer) * 2.0) : 0.0f, 64f)), null, Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, (float)((tileLocation.Y + 2.0) * 64.0 / 10000.0));
                    return false;
                }
                return true;
            }
